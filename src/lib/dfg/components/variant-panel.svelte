<script lang="ts">
   ...
   * selection; only Apply writes it through, and only then does the graph
   * rebuild.
   */
  ...
  import VariantRow from "$lib/dfg/components/variant-row.svelte";
  ...
  let {
    ...
    graph,
    onClose
  }: { project: Project; graph: ResponseDfg | null; onClose: () => void } = $props();
  ...
  const applied = $derived(selectedVariants().size);
  ...
  const onGraph = $derived.by(() => {
    if (!graph) return new Set<string>();
    return simplify(graph, view).variants.keys;
  });
