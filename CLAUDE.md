...
- `pnpm check` — type-check via `svelte-kit sync && svelte-check`, then `tsc -p e2e/tsconfig.json` for the e2e suite. Run this after any change. `tsconfig.json` excludes `**/*.test.ts`, so it does not type-check test files: a fixture built from a type that has since changed passes `check` and fails only under `pnpm test`. Run both.
